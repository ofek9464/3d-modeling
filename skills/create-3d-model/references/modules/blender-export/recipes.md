...
**glTF caveats**:
- Use exporter-supported material nodes and verify the target output. Procedural shaders are dropped or simplified.
- Example lightweight web profile: 15 MB maximum, 8 MB preferred; apply only when this target is requested.
...
| Procedural material missing in glTF | Bake to image textures first; use exporter-supported surface inputs |
...
| GLB exceeds the selected target budget | Apply Decimate (Recipe 2); reduce textures to 1024×1024 |
