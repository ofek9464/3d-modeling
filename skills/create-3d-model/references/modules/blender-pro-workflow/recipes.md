...
## Example sequence for a new hero scene
...
**Why this order**: include a stage only when its output is needed by a later stage. A great model under bad lighting renders worse than a mediocre model under great lighting. A perfect material tuned in flat lighting will look wrong once real lighting goes in.
...
Use this for a new scene that needs a blockout. Establishes scale, composition, camera framing.
...
5. **Change the diagnostic view** — compare a thumbnail, grayscale image, or mirrored preview when the current view obscures a defect.
...
- Comparing an independent diagnostic view
