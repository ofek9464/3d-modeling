...
For a hero shot intended to show the broad face, orient the asset and camera so that face is visible; preserve a different requested view. A sword viewed edge-on (camera looking down the thin axis) renders as a thin pole and looks nothing like a sword. The recipes below use this convention:
...
**1. Choose joins for the deliverable.** For a visual-only assembly, choose overlap relative to the joint scale; the 5–15mm example below assumes a large prop. For printing or physical assembly, use the required solid union, clearance, or fit and validate it. The hidden volume disappears inside the larger part, leaving no visible seam.
