...
Apply physically-based materials to objects. For portable surface materials, start with Principled BSDF and exporter-supported inputs. For render-only work, use shaders appropriate to the requested look; verify or bake unsupported effects for export.

## Starting metallic values

For a homogeneous uncoated material, start with Metallic 0 for a dielectric or 1 for a metal. Mixed, textured, coated, or deliberately stylized surfaces may need intermediate values; judge them against the material and target instead of enforcing a universal switch.
...
For a render-only emitter, this recipe uses ShaderNodeEmission connected to Material Output. For a portable material, use supported emission inputs or bake/adapt the effect and verify its export. The mesh becomes a light source itself (contributes to scene illumination in Cycles).
...
| Symptom | Fix |
|---------|-----|
| "Plasticky" metals | Check the material type, texture blend, lighting and intended metallic response |
