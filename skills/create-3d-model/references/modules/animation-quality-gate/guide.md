...
If an animation is rejected, preserve the baseline and diagnose before another rebuild:

- collect preview frames/contact sheet;
- identify the failed dimension(s);
- use `quality-refinement-autoloop` for evidence-based artifact repair if the cause remains unclear;
- select an existing repair method or investigate the missing technique;
- repair the animation and recheck the changed motion.
