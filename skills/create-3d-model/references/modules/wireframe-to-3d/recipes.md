...
Use only the selected example with the shared task contract. Retarget objects, units, paths, Blender APIs, quality settings and output limits before execution. These are optional patterns, not an automatic scene-wide procedure.
...
- **Front + side** → contour evidence for registration and subsequent surface/depth modeling; the helper does not solve the volume.
...
2. **Use the agreed canonical view.** Create an image-empty/plane/reference overlay in the same front orthographic camera used for validation. Align scale, centerline, and bounding circle before adding depth.
...
### Pattern: PBR material (Principled BSDF — a portable surface starting point)
...
4. File size must satisfy the requested target budget; 15 MB / 8 MB is a legacy lightweight-web example.
...
| Material missing in GLB | Used non-Principled-BSDF nodes | Rebuild material using exporter-supported surface inputs |

### Decimate example for a selected lightweight-web budget
...
- File size in MB compared with the requested budget
...
## Legacy lightweight-web example constraints
...
- **Legacy consumer profile**: this example uses JavaScript idle motion. Export rigs or other animation when requested and supported by the actual target.
...
**The orchestrator selects the missing stages** when the user asks for a "model of X" from a wireframe.
